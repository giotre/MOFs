...
class policy(object):
    ...
    def get_actions(self, mode, N, K, alpha):
        ...
        Z = 100
        temp_prov = np.zeros(Z)
        action_prov = np.zeros(Z)
        for j in range(Z):
            f = self.get_score(mode, self.predictor, self.training, alpha)
            temp_prov[j] = np.argmax(f)
            action_prov[j] = self.actions[int(temp_prov[j])]
        action = np.bincount(action_prov.astype(int)).argmax()
        
        A = []
        for i in range(len(action_prov)):
            if action_prov[i]==action:
                A.append(i)

        self.actions = self.delete_actions(int(temp_prov[A[0]]))
